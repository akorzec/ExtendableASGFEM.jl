...
function stokesPrimalPreconditioner(A0::ExtendableSparseMatrix{Tv, Ti}, B::ExtendableSparseMatrix{Tv, Ti}, nmodes, vsize) where {Tv, Ti}
    ...
    return StokesPrimalPreconditioner{Tv, typeof(LUS)}(LUS, DA, temp, nmodes, vsize)
end
...
Base.size(S::StokesPrimal) = S.nmodes * (S.vsize[1] + S.vsize[2]) # d fehlt?
...
function solve_stokes_primal!(SolutionSGFEM::SGFEVector, A0, A, B, b0, G, nmodes, bfac; atol = 1.0e-14, rtol = 1.0e-14)
    ...
    S = StokesPrimal{eltype(G), typeof(A0), typeof(b0), typeof(G)}(A0, A, B, G, nmodes, vsize)
    @info "...initializing Preconditioner"
    @time P = stokesPrimalPreconditioner(A0.entries, B.entries, nmodes, vsize)
